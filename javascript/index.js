...
// 브라우저에서 대부분 지원하지 않는 서식으로 최근에도 사용 하지 않음
console.log();
...
`, 5, 8, '출력완료');           /* 템플릿 리터럴 형식을 이용하면 이와같이 백틱(``)내에 키보드를 통하여 표현하는 모든 것이
                                  그대로 문자열 형태로 표현. -> 화이트 스페이스 모두 인정됨                                                           */
console.log(`
...
console.log('2000년 ' + 10 + '월 ' + 21 + '일 ' + true + null);   /* 자바와 같이 '+' 연산자에 대하여 유사한 결과를 확인
                                                                    ...
                                                                    것이지 타입 확장의 개념이 아님에 유의 -> 자바와 같은 형식이 아님!!! */

console.log(7 / 3);   // 일반 산술 연산 적용시에도 자바와 같이 동일 타입간 연산 결과가 동일 타입이
console.log(8 / 4);   // 나오는 것이 아니라 연산 결과 그대로가 반영되어 결과 산출., -> 실수까지 출력
console.log('\n-------------------------\n');
